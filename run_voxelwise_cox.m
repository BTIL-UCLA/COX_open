...
%           5, 182, 218, 182, 6, 'yes');
    ...
    try
        ...
        if num_covariates == 6
            var_map = zeros(max_x, max_y, 3, max_z);  % var_cov4, var_cov6, covar(4,6) 
        end
    catch 
    ...
    for current_s = 1:max_z
        ...
        if  strcmpi(use_parallel,'yes')
            ...
            parfor i = 1:num_vox_thresh
                [par_b_temp(i,:), par_p_map_temp(i,:), par_var_covar_cov4_and_cov6(i,:)] = ...
                    processVoxel(i, age, MGMT, flair_vol, tx, tumor_array, ...
                                 num_covariates, interaction_var, T, status);
            end
        else
...
% ===============================
% Helper function (loop body)
% ===============================
function [b_out, p_out, varCov_out] = processVoxel(i, age, MGMT, flair_vol, tx, tumor_array, ...
                                                   num_covariates, interaction_var, T, status)
    ...
    % Output beta coefficients and p-values
    b_out = b(:)';
    p_out = stats.p(:)';

    % Handle variance/covariance
    if num_covariates == 6
        cov_matrix = stats.covb;
        var_cov4 = cov_matrix(4,4);
        var_cov6 = cov_matrix(6,6);
        cov_cov4_6 = cov_matrix(4,6);
        varCov_out = [var_cov4, var_cov6, cov_cov4_6];
    else
        varCov_out = nan(1,3); % placeholder if not used
    end
